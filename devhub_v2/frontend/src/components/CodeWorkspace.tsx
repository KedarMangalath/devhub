...
export default function CodeWorkspace({ workspaceId, projectId, projectPath, onProjectChanged }: Props) {
  const [activeSidePanel, setActiveSidePanel] = useState<'files' | 'search' | 'chat'>('files');
  ...
  const previewUrl = runtime?.preview_url || (runtime?.run_command ? `http://127.0.0.1:${getPort(runtime.run_command)}` : null);
  ...
  const fetchRuntime = () => {
    if (!workspaceId) return;
    fetch(`${API}/workspace/${workspaceId}/runtime/`).then((r) => r.json()).then(setRuntime);
  };
  ...
  useEffect(() => chatEndRef.current?.scrollIntoView({ behavior: 'smooth' }), [chatMessages, activeSidePanel]);
  ...
  const sendChat = async () => {
    ...
    try {
      const response = await fetch(`${API}/projects/${projectId}/chat/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ content, selected_file: selectedFile, selected_content: selectedFile ? fileContent : '' }),
      });
      const data = await response.json();
      setChatMessages((current) => [...current, { role: 'assistant', content: data.assistant_message ?? 'No response.' }]);
      ...
      setChatMessages((current) => [...current, { role: 'assistant', content: 'Connection failed.' }]);
    } finally {
  ...
  return (
    <div className="flex h-full w-full min-h-0 min-w-0 overflow-hidden rounded-lg border border-[#333333] bg-[#1e1e1e] text-[#cccccc]">
      <div className="z-10 flex w-12 shrink-0 flex-col items-center border-r border-[#333333] bg-[#252526] py-2">
        ...
        <button onClick={() => { setActiveSidePanel('chat'); setShowSidePanel(true); }} className={`group relative mb-2 rounded-lg p-2.5 ${activeSidePanel === 'chat' && showSidePanel ? 'text-white' : 'text-[#858585] hover:text-white'}`}><Bot className="h-5 w-5" />{activeSidePanel === 'chat' && showSidePanel && <div className="absolute left-0 top-1/2 h-6 w-0.5 -translate-y-1/2 bg-[#007acc]" />}</button>
        <div className="flex-1" />
      </div>

      {showSidePanel && (
        <div className="flex w-[min(22rem,38vw)] min-w-[17rem] max-w-[22rem] shrink-0 flex-col border-r border-[#333333] bg-[#252526]">
          <div className="flex h-[35px] items-center justify-between px-4 text-xs font-semibold uppercase tracking-wide text-[#BBBBBB]">
            {activeSidePanel === 'files' ? 'Explorer' : activeSidePanel === 'search' ? 'Search' : 'AI Assistant'}
          ...
          <div className="flex-1 min-h-0 overflow-hidden">
            ...
            {activeSidePanel === 'chat' && <div className="flex h-full min-h-0 flex-col"><div className="border-b border-[#333333] px-4 py-3"><div className="flex items-center gap-2 text-[11px] font-semibold text-emerald-400"><Sparkles className="h-3.5 w-3.5" />DevHub Chat</div><p className="mt-1 text-[11px] leading-5 text-[#858585]">Ask for UI changes, code edits, or feature work directly against this workspace.</p></div><div className="flex-1 min-h-0 overflow-y-auto px-4 py-4"><div className="space-y-4">{chatMessages.map((message, index) => <div key={index} className={message.role === 'user' ? 'ml-6 rounded-2xl bg-[#3c3c3c] px-3 py-2.5 text-xs text-white' : 'pr-4 text-xs text-[#cccccc]'}>{message.role === 'assistant' && <div className="mb-1 flex items-center gap-1.5 font-semibold text-emerald-400"><Bot className="h-3.5 w-3.5" />DevHub</div>}<div className="whitespace-pre-wrap leading-relaxed">{message.content}</div></div>)}{chatSending && <div className="flex items-center gap-2 text-xs text-[#858585]"><Loader2 className="h-3.5 w-3.5 animate-spin text-emerald-500" />Thinking...</div>}<div ref={chatEndRef} /></div></div><div className="border-t border-[#333333] p-3">{selectedFile && <div className="mb-2 flex items-center gap-1 px-1 text-[9px] text-emerald-400/80"><FileText className="h-2.5 w-2.5" />Context: {selectedFile.split(/[\\/]/).pop()}</div>}<div className="relative flex"><textarea value={chatInput} onChange={(event) => setChatInput(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); sendChat(); } }} placeholder="Ask me anything..." className="min-h-[44px] w-full resize-none rounded-xl border border-transparent bg-[#3c3c3c] py-2 pl-3 pr-10 text-xs text-white outline-none focus:border-[#007acc]" rows={1} /><button onClick={sendChat} disabled={!chatInput.trim() || chatSending} className="absolute bottom-1.5 right-1.5 rounded-lg bg-[#2d2d2d] p-1.5 text-emerald-400 hover:bg-[#4d4d4d] hover:text-emerald-300 disabled:opacity-30"><Send className="h-3.5 w-3.5" /></button></div></div></div>}
          </div>
        </div>
      )}
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-[#1e1e1e]">
        ...
        <div className="z-20 flex h-5 shrink-0 items-center justify-between bg-[#007acc] px-3 text-[10px] text-white">
          <div className="flex items-center gap-3"><span className="flex items-center gap-1 rounded px-1 hover:bg-white/20"><Wrench className="h-3 w-3" />DevHub IDE</span>{selectedFile && <span>{getLanguage(selectedFile).toUpperCase()}</span>}{runtime?.status?.running && <span className="flex items-center gap-1 px-1"><span className="h-1.5 w-1.5 rounded-full bg-green-300 animate-pulse" />Port {getPort(runtime.run_command)}</span>}</div>
