  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        />
        <div className={`p-4 rounded-full shadow-sm mb-4 transition-colors ${isDragging ? 'bg-vacb-100' : 'bg-white'}`}>
          <UploadCloud className={`w-8 h-8 ${isDragging ? 'text-vacb-700' : 'text-vacb-600'}`} />
        </div>
              ...
              >
                <div className="flex items-center space-x-3 overflow-hidden">
                  <div className="bg-slate-50 p-2 rounded-md shrink-0 border border-slate-100">
                    <FileText className="w-5 h-5 text-vacb-600" />
                  </div>
      ...
      {/* Security Note */}
      <div className="bg-vacb-50 border border-vacb-100 rounded-lg p-4 flex items-start space-x-3">
        <div className="shrink-0 mt-0.5">
          <svg className="w-5 h-5 text-vacb-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
          </svg>
        </div>
        <div>
          <h5 className="text-sm font-medium text-vacb-900">Secure & Immutable</h5>
          <p className="text-xs text-vacb-700 mt-1 leading-relaxed">
