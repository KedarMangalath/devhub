...
const getIcon = (name, index) => {
  ...
  if (lowerName.includes('rate')) return <ShieldCheck className="w-6 h-6 text-vacb-700" />;
  if (lowerName.includes('time') || lowerName.includes('avg')) return <Clock className="w-6 h-6 text-vacb-700" />;
  if (lowerName.includes('case') || lowerName.includes('processed')) return <FileText className="w-6 h-6 text-vacb-700" />;
  if (lowerName.includes('audit') || lowerName.includes('log')) return <Database className="w-6 h-6 text-vacb-700" />;

  const icons = [
    <ShieldCheck className="w-6 h-6 text-vacb-700" />,
    <Clock className="w-6 h-6 text-vacb-700" />,
    <FileText className="w-6 h-6 text-vacb-700" />,
    <Database className="w-6 h-6 text-vacb-700" />
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {displayData.map((stat, index) => (
            ...
            >
              <div className="p-3 bg-vacb-50 rounded-full mb-4 ring-4 ring-vacb-50/50">
