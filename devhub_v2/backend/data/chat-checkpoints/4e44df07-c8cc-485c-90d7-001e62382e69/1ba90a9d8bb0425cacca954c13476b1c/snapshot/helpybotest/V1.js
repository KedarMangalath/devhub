(function() {
    ...
    {% comment %} function formatMessage(message, isBot) {
        if (isBot) {
            // First, identify and clean up the image section with label
            message = message.replace(/!\[Image\]\(([^)]+)\)/g, function(match, src) {
                return `\n<div class="message-image-container"><img src="${src}" alt="Product Image" loading="lazy" onerror="this.style.display='none'"/></div>\n`;
            });
