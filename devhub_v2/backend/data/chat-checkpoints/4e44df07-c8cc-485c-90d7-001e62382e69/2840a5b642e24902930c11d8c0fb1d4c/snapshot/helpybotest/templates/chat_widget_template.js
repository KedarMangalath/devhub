(function() {
    ...
    let leadCollectionInProgress = false; // Flag to track lead collection state
    let leadState = {
        nameCollected: false,
    ...
    // Log lead collection status for debugging
    console.log("Lead collection enabled status:", leadsEnabled);
...
#chat-widget .bot-message:before {
    content: "";
    z-index: -1;
    position: absolute;
    top: 0;
...
#chat-widget .bot-message::after {
    ...
    border-radius: inherit;
        ...
        #chat-widget .share-options button {
            background: none;
            border: none;
            padding: 5px;
            cursor: pointer;
        ...
        #chat-widget .share-options button:hover {
...
@keyframes slideOut {
    ...
    to {
        ...
        #chat-widget .page::-webkit-scrollbar {
            width: 8px;
        }

        #chat-widget .page::-webkit-scrollbar-track {
            background:rgb(255, 255, 255);
            border-radius: 10px;
        }

        #chat-widget .page::-webkit-scrollbar-thumb {
            background-color: {{ header_color }};
            border-radius: 10px;
            border: 2px solid #e0e0e0;
        }

        #chat-widget .page::-webkit-scrollbar-thumb:hover {
            background-color: {{ header_color }};
        }
...
#minimize-chat:hover i {
    ...
    color: rgba(255, 255, 255, 0.8); /* Slightly fade the icon color */
        ...
        #chat-widget .card img, #chat-widget .card iframe {
            width: 100%;
            height: 100%;
            object-fit: cover;
            border-radius: 15px;
        ...
        @media (max-width: 768px) {
            ...
            #chat-button-container {
                bottom: 10px !important; /* Keeps it at the bottom */
                left: 50% !important; /* Positions the left edge at center of screen */
                right: unset; /* Removes right positioning */
...
.parent:hover #chat-widget .card {
  transform: rotate3d(1, 1, 0, 20deg);
  box-shadow: rgba(5, 71, 17, 0.3) 30px 50px 25px -40px, rgba(5, 71, 17, 0.1) 0px 25px 30px 0px;
}

.parent:hover #chat-widget .card .bottom .social-buttons-container .social-button {
  transform: translate3d(0, 0, 50px);
  box-shadow: rgba(5, 71, 17, 0.2) -5px 20px 10px 0px;
}

.parent:hover #chat-widget .card .logo .circle2 {
  transform: translate3d(0, 0, 60px);
}

.parent:hover #chat-widget .card .logo .circle3 {
  transform: translate3d(0, 0, 80px);
}

.parent:hover #chat-widget .card .logo .circle4 {
  transform: translate3d(0, 0, 100px);
}

.parent:hover #chat-widget .card .logo .circle5 {
...
.send-button:disabled {
    ...
    {% comment %} async function fetchActiveFlow() {
        try {
            // Use the specific base URL instead of window.location.host
            const baseUrl = "onemeraki.helpybo.com";
            // Construct the full URL with https
            const url = `http://${baseUrl}/api/get-active-flow/${chatbotId}/`;
            ...
            if (data.success && data.has_flow) {
                activeFlow = data.flow;
                // Make sure we have a lead_collection_step property
                if (typeof activeFlow.lead_collection_step === 'undefined') {
                    activeFlow.lead_collection_step = activeFlow.steps.length - 1;
                }
                console.log("Active flow set:", activeFlow);
            ...
            console.error('Error fetching active flow:', error);
            // If flow fetch fails, continue without flow
            return false;
        }
    } {% endcomment %}
    ...
    function toggleChat() {
        ...
        if (chatWidget.style.display === 'none' || chatWidget.style.display === '') {
            ...
            userMessageCount = 0;
            currentStep = 0;
            leadCollectionInProgress = false; // Keep this as false initially
            inActiveFlow = false; // Reset flow state
            leadState = {
                nameCollected: false,
                phoneCollected: false,
                name: '',
    ...
    function appendMessage(content, className, isHTML = false) {
        ...
        if (className === 'user-message') {
            ...
            // ONLY check if we should trigger lead collection when NOT in an active flow
            if (userMessageCount >= 3 && !leadState.nameCollected && leadsEnabled && 
                !leadCollectionInProgress && !inActiveFlow) { // Added !inActiveFlow check
    ...
    function updateInputState() {
        ...
        // Disable input if leads are required but not fully collected
        if (leadsEnabled && (!leadState.nameCollected || !leadState.phoneCollected)) {
            ...
            messageInput.placeholder = messageInput.getAttribute('data-original-placeholder') || 'Type a message...';
        }
    }
    ...
    function sendMessage() {
        ...
        // IMPORTANT: Check for lead collection before sending message
        if (leadsEnabled && (!leadState.nameCollected || !leadState.phoneCollected)) {
    ...
    function processFlowStep(stepIndex) {
        ...
        // Set the inActiveFlow flag to true when processing steps
        inActiveFlow = true;
        ...
        if (!activeFlow || !activeFlow.steps || stepIndex >= activeFlow.steps.length) {
            ...
            // Flow is now complete - set flag to false
            inActiveFlow = false;
            
            // Begin lead collection ONLY when flow ends completely
            if (leadsEnabled && !leadState.nameCollected && !leadCollectionInProgress) {
                console.log("Starting lead collection after flow completion");
                setTimeout(() => {
                    leadCollectionInProgress = true;
                    handleLeadCollection();
                }, 1000);
            }
            return;
        ...
        // If it's a buttons step, add buttons
        ...
        // Auto-progress for message steps
...
// FIXED: Improved handleLeadCollection function
function handleLeadCollection() {
    console.log("Lead collection attempt:", 
                "leadsEnabled:", leadsEnabled,
                "inProgress:", leadCollectionInProgress,
                "nameCollected:", leadState.nameCollected);
    
    // Don't return early if already in progress - reset the flag
    leadCollectionInProgress = true;
...
function createLeadForm(type) {
    ...
    // Clean up any existing forms to prevent duplicates
    document.querySelectorAll('.lead-form').forEach(form => form.remove());
    ...
    form.style.border = `2px solid {{ header_color }}`;
    form.style.borderRadius = "10px";

    if (type === 'name') {
        form.innerHTML = `
            <input type="text" class="lead-input" placeholder="Enter your name" style="width: 100%; padding: 8px; margin-bottom: 10px; border: 1px solid {{ header_color }}; border-radius: 5px;">
            <button class="lead-submit-btn" style="background-color: {{ header_color }}; color: white; padding: 8px 16px; border: none; border-radius: 5px; cursor: pointer;">Submit</button>
        `;
        ...
        submitBtn.onclick = () => {
            ...
            if (name) {
                ...
                appendMessage(name, 'user-message');
                
                // Remove this form
                form.remove();
                
                // Create phone form after a short delay
                setTimeout(() => {
                    handleLeadCollection();
                }, 500);
            } else {
        ...
        form.innerHTML = `
            <input type="tel" class="lead-input" placeholder="Enter your 10-digit phone number" style="width: 100%; padding: 8px; margin-bottom: 10px; border: 1px solid {{ header_color }}; border-radius: 5px;">
            <button class="lead-submit-btn" style="background-color: {{ header_color }}; color: white; padding: 8px 16px; border: none; border-radius: 5px; cursor: pointer;">Submit</button>
        `;
        ...
        submitBtn.onclick = () => {
            ...
            if (/^\d{10}$/.test(phone)) {
                ...
                // Show user's input as a message
                appendMessage(phone, 'user-message');
    ...
    // Update the input state to prevent regular chat input
    updateInputState();
}
...
function saveLead() {
    ...
    .then(data => {
        if (data.success) {
            ...
            appendMessage(LEAD_MESSAGES.THANK_YOU, 'bot-message');
            
            // Lead collection is now complete
            leadCollectionInProgress = false;
    ...
    .catch(error => {
        ...
        // Reset lead collection state on error
        leadCollectionInProgress = false;
        updateInputState();
    });
}
...
// 3. Modify handleButtonClick to properly set the flag when flow ends
function handleButtonClick(event) {
    ...
    if (activeFlow && activeFlow.steps && stepIndex < activeFlow.steps.length - 1) {
        ...
        // This is the last step, mark flow as complete
        inActiveFlow = false;
        
        // ONLY NOW trigger lead collection if this is the last step
        if (isLastStep && leadsEnabled && !leadState.nameCollected && !leadCollectionInProgress) {
            setTimeout(() => {
                leadCollectionInProgress = true;
                handleLeadCollection();
            }, 1000); // Delay to ensure it happens after flow completion
        }
