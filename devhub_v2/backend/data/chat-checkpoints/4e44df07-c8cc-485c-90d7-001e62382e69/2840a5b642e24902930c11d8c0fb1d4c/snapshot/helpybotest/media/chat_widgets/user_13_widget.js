...
#chat-widget #chat-widget .bot-message:before {
    ...
    bottom: 0;
    left: 0;
        background: linear-gradient(-45deg, #fdb926 0%, #fdb926 100%);
...
#chat-widget #chat-widget .bot-message::after {
    ...
    z-index: 10;
        ...
        #chat-button:before {
            ...
            bottom: 0;
            left: 0;
            background: linear-gradient(-45deg, #fdb926 0%, #ffffff 100%);
...
.quick-link:hover:before {
  top: -70%;
  background-image: radial-gradient(circle, #fdb926; 20%, transparent 20%),
    radial-gradient(circle, transparent 20%, #fdb926; 20%, transparent 30%),
    radial-gradient(circle, #fdb926; 20%, transparent 20%),
    radial-gradient(circle, #fdb926; 20%, transparent 20%),
    radial-gradient(circle, transparent 10%, #fdb926; 15%, transparent 20%),
    radial-gradient(circle, #fdb926; 20%, transparent 20%),
    radial-gradient(circle, #fdb926; 20%, transparent 20%),
    radial-gradient(circle, #fdb926; 20%, transparent 20%),
    radial-gradient(circle, #fdb926; 20%, transparent 20%);
...
width:100%;
  height: 100%;
  border-radius: 55px;
  background: linear-gradient(135deg, #fdb926, #fdb926);
...
.send-button:disabled {
    ...
    // Add emergency widget function
    function addEmergencyWidget() {
        console.log("Adding emergency chat button");
        ...
        `;
        emergencyButton.innerHTML = '<i class="fas fa-comment"></i>';
        emergencyButton.onclick = toggleChat;
        document.body.appendChild(emergencyButton);
    }

    // Update the fetchActiveFlow function to support lead collection configuration
        ...
        chatWidget.innerHTML = `
            <div id="chat-header">
                <div class="header-text-container">
                    <img id="chat-logo" src="${'http://127.0.0.1:8000/media/chatbot_logos/Screenshot_2024-12-30_142643_wrmWXKd.png' || '#'}" alt="Chatbot Logo">
                        ...
                        <div class="card">
                            <div class="logo">
                                ...
                                <span class="circle circle5">
                                    <img src="${'http://127.0.0.1:8000/media/chatbot_logos/Screenshot_2024-12-30_142643_wrmWXKd.png' || '#'}" alt="Chatbot Logo" height="35">
                                </span>
                            </div>
                            <div class="glass"></div>
                            <div class="content">
            ...
            <div id="chat-footer">
                <ul class="footer-button">
                    <li style="--i:#fdb926;--j:#fdb926;" onclick="window.navigateToPage('home')">
                ...
                <ul class="footer-button">
                    <li style="--i:#fdb926;--j:#fdb926;" onclick="window.navigateToPage('chat')">
                ...
                <ul class="footer-button">
                    <li style="--i:#fdb926;--j:#fdb926;" onclick="window.navigateToPage('help')">
    ...
    function toggleChat() {
        ...
        if (chatWidget.style.display === 'none' || chatWidget.style.display === '') {
            ...
            fetch('http://127.0.0.1:8000/chat/response/13/', {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    logo_url: 'http://127.0.0.1:8000/media/chatbot_logos/Screenshot_2024-12-30_142643_wrmWXKd.png'
                })
            })
    ...
    function appendMessage(content, className, isHTML = false) {
        ...
        if (className === 'user-message') {
            ...
            if (userMessageCount === 4 && !leadState.nameCollected && leadsEnabled) {
    ...
    function processFlowStep(stepIndex) {
        ...
        if (!activeFlow || !activeFlow.steps || stepIndex >= activeFlow.steps.length) {
            ...
            // Begin lead collection when flow ends
    ...
    function createLeadForm(type) {
        ...
        // Remove any existing lead forms
...
// Replace the saveLead function with this implementation
function saveLead() {
    ...
    const chatResponseUrl = 'http://127.0.0.1:8000/chat/response/13/';
...
// Replace the handleButtonClick function with this implementation
function handleButtonClick(event) {
    ...
    // Check if we should trigger lead collection based on flow progress
    const isLastStep = (activeFlow && stepIndex >= activeFlow.steps.length - 1);
    
    if ((isLastStep || stepIndex === activeFlow?.lead_collection_step) && 
        !leadState.nameCollected && leadsEnabled) {
        setTimeout(() => {
            appendMessage("Before we continue, could you please tell me your name?", 'bot-message');
            createLeadForm('name');
    ...
    function initAll() {
        ...
        // Force enable leads collection if needed
        leadsEnabled = true;
        ...
        Promise.all([...resources.map(loadResource), fetchActiveFlow()])
            ...
            .catch(error => {
                ...
                // If normal initialization fails, use emergency fallback
                addEmergencyWidget();
            });
    }
    ...
    // Force enable emergency button after 5 seconds if widget isn't visible
    setTimeout(function() {
        if (!document.getElementById('chat-widget') || 
