(function() {
    ...
    let leadCollectionInProgress = false; // New flag to track lead collection state
    let leadState = {
        nameCollected: false,
    ...
    function triggerLeadCollection() {
        ...
        if (leadCollectionInProgress) return; // Don't trigger if already in progress
    ...
    function checkAndTriggerLeadCollection() {
        ...
        if (!leadState.nameCollected && !leadCollectionInProgress) {
    ...
    function handleLeadCollection() {
        // Prevent multiple lead collection processes
        if (leadCollectionInProgress) return true;
        
        // Set the flag to indicate lead collection is in progress
        leadCollectionInProgress = true;
        console.log("Lead collection started");
...
#chat-widget .bot-message:before {
    ...
    bottom: 0;
    left: 0;
        background: linear-gradient(-45deg, #d9cc3f 0%, #d9cc3f 100%);
...
#chat-widget .bot-message::after {
    ...
    z-index: 10;
        ...
        #chat-button:before {
            ...
            bottom: 0;
            left: 0;
            background: linear-gradient(-45deg, #d9cc3f 0%, #ffffff 100%);
...
.quick-link:hover:before {
  top: -70%;
  background-image: radial-gradient(circle, #d9cc3f; 20%, transparent 20%),
    radial-gradient(circle, transparent 20%, #d9cc3f; 20%, transparent 30%),
    radial-gradient(circle, #d9cc3f; 20%, transparent 20%),
    radial-gradient(circle, #d9cc3f; 20%, transparent 20%),
    radial-gradient(circle, transparent 10%, #d9cc3f; 15%, transparent 20%),
    radial-gradient(circle, #d9cc3f; 20%, transparent 20%),
    radial-gradient(circle, #d9cc3f; 20%, transparent 20%),
    radial-gradient(circle, #d9cc3f; 20%, transparent 20%),
    radial-gradient(circle, #d9cc3f; 20%, transparent 20%);
...
width:100%;
  height: 100%;
  border-radius: 55px;
  background: linear-gradient(135deg, #d9cc3f, #d9cc3f);
        ...
        chatWidget.innerHTML = `
            <div id="chat-header">
                <div class="header-text-container">
                    <img id="chat-logo" src="${'http://127.0.0.1:8000/media/chatbot_logos/oneeeee.PNG' || '#'}" alt="Chatbot Logo">
                    <div data-original-text="ONE MERAKI">ONE MERAKI</div>
                </div>
                        ...
                        <div class="card">
                            <div class="logo">
                                ...
                                <span class="circle circle5">
                                    <img src="${'http://127.0.0.1:8000/media/chatbot_logos/oneeeee.PNG' || '#'}" alt="Chatbot Logo" height="35">
                                </span>
                            </div>
                            <div class="glass"></div>
                            <div class="content">
                                <span class="title">ONE MERAKI</span>
...
<button id="send-button"><i class="fas fa-paper-plane"></i></button>
            ...
            <div id="chat-footer">
                <ul class="footer-button">
                    <li style="--i:#d9cc3f;--j:#d9cc3f;" onclick="window.navigateToPage('home')">
                ...
                <ul class="footer-button">
                    <li style="--i:#d9cc3f;--j:#d9cc3f;" onclick="window.navigateToPage('chat')">
                ...
                <ul class="footer-button">
                    <li style="--i:#d9cc3f;--j:#d9cc3f;" onclick="window.navigateToPage('help')">
    ...
    function toggleChat() {
        ...
        if (chatWidget.style.display === 'none' || chatWidget.style.display === '') {
            ...
            // Clear previous chat and reset all state
            document.getElementById('chat-messages').innerHTML = '';
            
            // Reset all state variables when opening the chat
            userMessageCount = 0;
            currentStep = 0;
            ...
            // Initialize session
            fetch('http://127.0.0.1:8000/chat/response/6/', {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    logo_url: 'http://127.0.0.1:8000/media/chatbot_logos/oneeeee.PNG'
                })
            })
    ...
    function updateInputState() {
        ...
        if (leadsEnabled && leadCollectionInProgress) {
    ...
    function sendMessage() {
        ...
        // Don't process more messages if lead collection is in progress
        if (leadCollectionInProgress) {
            return;
        }
        ...
        .then(data => {
            ...
            // Check if we should trigger lead collection after certain number of messages
            if (userMessageCount >= 3 && leadsEnabled && !leadCollectionInProgress && 
                !leadState.nameCollected && !activeFlow) {
                setTimeout(() => {
                    triggerLeadCollection();
                }, 1000);
            }
    ...
    // Flow step processing - this is the key function that was causing issues
    function processFlowStep(stepIndex) {
        console.log("Processing flow step:", stepIndex);
        
        // Don't process steps if lead collection is in progress
        if (leadCollectionInProgress) {
            console.log("Lead collection in progress, skipping flow step");
            return;
        }
        ...
        if (!activeFlow || !activeFlow.steps || stepIndex >= activeFlow.steps.length) {
            ...
            // After flow completes, check if lead collection should start
        ...
        if (step.type === 'buttons' && step.buttons && step.buttons.length > 0) {
            ...
            // We do NOT proceed automatically - we wait for button click
        } 
    ...
    // Button click handler - this is the second function that was causing issues
    function handleButtonClick(event) {
        ...
        // IMPORTANT: Check if we should move to the next step or collect lead info
        const isLastStep = (activeFlow && stepIndex >= activeFlow.steps.length - 1);
        
        // Move to next step if we're not at the end of the flow
        if (!isLastStep) {
            setTimeout(() => {
                processFlowStep(stepIndex + 1);
                isProcessingClick = false;
        ...
        // If this was the last step and leads are enabled, start lead collection
        else if (isLastStep && leadsEnabled && !leadState.nameCollected) {
            setTimeout(() => {
                leadCollectionInProgress = true;
                handleLeadCollection();
                isProcessingClick = false;
    ...
    // Lead form creation function
    function createLeadForm(type) {
        console.log("Creating lead form:", type);
        
        // First, mark lead collection as in progress
        leadCollectionInProgress = true;
        
        // Update input state to disable normal chat input during form
        updateInputState();
    ...
    function saveLead() {
        ...
        const chatResponseUrl = 'http://127.0.0.1:8000/chat/response/6/';
        ...
        .catch(error => {
            ...
            // Lead collection failed, so reset 
            leadCollectionInProgress = false;
